...
- **Gallery tiles show the game name only on hover, over a darkened cover.** The name row beneath each cover is gone; tiles are now cover-only. Hovering a tile dims the whole cover and reveals the title centred over it (Steam/console-launcher style). The selection checkbox moved inside the cover at the bottom-left — shown on hover while unchecked, always visible once checked — and the platform pill moved to the bottom-right. The DOWNLOADED banner and platform pill stay visible at all times.
...
- **Long game titles no longer overlap the cover art in the gallery grid.** Tile name labels had word-wrap enabled but no height bound, so a long title's wrapped height overflowed the fixed-height cell and squeezed the fixed-height cover above it — painting the name on top of the art. The name row is now capped at `NAME_LINES` (2) lines (`setFixedHeight` from font line-spacing); longer titles clip and show the full name on hover via a tooltip.
